...
_SECRET_PATTERNS = [
    ...
    # Catches: password="x", "password": "x", api_key=x, token: 'x', etc.
    re.compile(
        r"""(?ix)
        ["']?\b(password|passwd|pwd|secret|token|api[-_]?key|access[-_]?key|bearer)\b["']?
        \s*[:=]\s*
        ["']?[^"',}\s][^"',}\s]*["']?
        """
    ),
    ...
    re.compile(r"(?<![A-Za-z0-9_\-])eyJ[A-Za-z0-9_\-\.]{20,}(?![A-Za-z0-9_\-])"),
