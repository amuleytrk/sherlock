...
1. The frontend sends `env: "ppe"` in each chat request.
2. `apps/api/main.py` sets `active_env` (a ContextVar) before dispatching to
   any agent or tool.
3. MCP servers (postgres, cosmos, redis, kubectl) read the active env, ask
...
(e.g. `PG_PPE_HOST`). `env_config()` does the dynamic lookup so settings
